...
The Catalog section of the app answers one question about the whole store:
which active products carry which `custom.routine_step` value. It is a bulk
operation rather than a paginated loop, and the reasoning is
[ADR-0004](adr/0004-bulk-operations-over-pagination.md).

What happens when the button is pressed:

1. `POST /api/catalog/export` queues a `catalog.export` job and answers 202. If
   one is already in flight it queues nothing and returns the running one.
2. The worker's first attempt calls `bulkOperationRunQuery` and **writes the
   operation id to the job row before polling anything**. Starting a bulk
   operation is not idempotent; without that a retry would start a second
   export of the same catalog.
3. Each attempt polls `bulkOperation(id:)` for at most thirty seconds. Running
   out of that budget is not a failure — the job goes back to PENDING with the
   attempt returned, and the sync log shows it pending rather than red.
4. When Shopify reports COMPLETED, the JSONL result is streamed from the signed
   URL, counted, and the summary is stored in `Job.result`.

Two traps worth knowing before touching this code:

- **`currentBulkOperation` is deprecated** as of 2026-01 and is also the wrong
  question — an app may have five operations running at once on a shop. Use
  `bulkOperation(id:)` with the id the mutation returned.
- **The result URL expires after seven days**, and so does `partialDataUrl` on a
  failed operation. A report that has to be regenerated is regenerated by
  running the export again, not by re-reading an old file.

Watching one locally, with the app running and a store connected:

```bash
pnpm dev | grep '"jobKind":"catalog.export"'
```

The log lines are `Catalog export started`, `Catalog export still running` with
the object count so far, and `Catalog export finished`. All three carry the same
`correlationId`, which for a UI-triggered job is a `ui-` prefixed UUID rather
than a Shopify delivery id.
